...
def main():
    ...
    parser.add_option('-i', '--interface', dest='port', help='SocketCAN interface (example: can0)', metavar='IFACE')
    ...
    parser.add_option('-d', '--device', dest='device', help='target device', metavar='DEV')

    parser.add_option('--req-id', dest='req_id', default='0x600', help='request CAN ID', metavar='CANID')
    parser.add_option('--resp-id', dest='resp_id', default='0x650', help='response CAN ID', metavar='CANID')
    parser.add_option('--fd', dest='fd', default=False, action='store_true', help='use CAN FD frames')
    parser.add_option('--extid', dest='extid', default=False, action='store_true', help='use extended CAN IDs')
    ...
    if options.swap:
        if device not in ("SAME5X", "SAMD5X", "PIC32MZ", "PIC32CX_SG41", "PIC32MK", "PIC32CX_MT", "PIC32CZ"):
    ...
    for cnt in range(0, count):
        ...
        if options.file and ((count == 2 and cnt == 1) or (count == 1 and cnt == 0)):
            ...
            resp = transport.send_request(BL_CMD_UNLOCK, uint32(8), uint32(address) + uint32(size))
        ...
        for idx, blk in enumerate(blocks):
            ...
            if (options.devCfgBinfile is not None) and count == 2 and cnt == 0:
                ...
                resp = transport.send_request(BL_CMD_UNLOCK, uint32(8), uint32(addr) + uint32(data_length))
            ...
            if (options.devCfgBinfile is not None) and count == 2 and cnt == 0:
                ...
                for x in addr_range_list:
                    ...
                    if (addr >= devCfgStartAddr and addr < devCfgEndAddr):
                        resp = transport.send_request(BL_CMD_DEVCFG_DATA, uint32(data_length + 4), uint32(addr) + blk)
                        break
            ...
            if (options.devCfgBinfile is not None) and count == 2 and cnt == 0:
                ...
                resp = transport.send_request(BL_CMD_VERIFY, uint32(4), uint32(crc_chunk))
            ...
            if (options.file is not None) and ((count == 2 and cnt == 1) or (count == 1 and cnt == 0)):
                resp = transport.send_request(BL_CMD_DATA, uint32(data_length + 4), uint32(addr) + blk)
        ...
        if options.swap is True:
            ...
            resp = transport.send_request(BL_CMD_BKSWAP_RESET, uint32(16), uint32(0) * 4)
        else:
