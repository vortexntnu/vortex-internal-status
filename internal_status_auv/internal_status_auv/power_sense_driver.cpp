...
PowerSenseDriver::PowerSenseDriver(std::uint8_t i2c_bus,
    ...
    if (bus_fd_ < 0) {
        throw std::runtime_error("ERROR: Failed to open I2C bus " +
    ...
    try {
        ...
        if (write(bus_fd_, &config_channel_0, i2c_data_size) != i2c_data_size) {
            throw std::runtime_error(
                "Error: Failed to write to I2C device for Channel 0 : " +
                std::string(strerror(errno)));
            return false;
        }

        // Delay between channel switches (MCP342X requires some time for
        // settling)
        ...
        if (write(bus_fd_, &config_channel_1, i2c_data_size) != i2c_data_size) {
            throw std::runtime_error(
                "Error: Failed to write to I2C device for Channel 1 : " +
                std::string(strerror(errno)));
            return false;
        }

    } catch (const std::exception& e) {
        std::cerr << "ERROR: Failed to start conversion - " << e.what()
                  << std::endl;
        return false;
    } catch (...) {
        std::cerr << "ERROR: Failed to start conversion - unknown error"
                  << std::endl;
        return false;
    }
...
std::uint16_t PowerSenseDriver::read_powersense(std::uint8_t channel) {
    try {
        constexpr std::size_t i2c_data_size =
            3;  // MCP342X returns 3 bytes (MSB, LSB, Config)
        ...
        // Configure MCP342X to select channel and start conversion
        std::uint8_t config =
            (channel & MCP342X_CHANNEL_MASK) |  // Select Channel
            MCP342X_RDY |                       // Start conversion
            resolution_;                        // Keep resolution

        if (write(bus_fd_, &config, 1) != 1) {
            throw std::runtime_error(
                "Error: Failed to write config to MCP342X: " +
                std::string(strerror(errno)));
            return;
        }

        // Wait for conversion to complete (MCP342X auto-clears RDY when ready)
        std::this_thread::sleep_for(std::chrono::milliseconds(5));

        // Read ADC result (3 bytes: MSB, LSB, Config)
        if (read(bus_fd_, i2c_data_array.data(), i2c_data_size) !=
            i2c_data_size) {
            throw std::runtime_error("Error: Failed to read from MCP342X: " +
                                     std::string(strerror(errno)));
            return;
        }

        // Convert received bytes to a 16-bit ADC value
        std::uint16_t adc_value = (i2c_data_array[0] << 8) | i2c_data_array[1];
        return adc_value;
        // Store ADC reading in output array

    } catch (const std::exception& e) {
        std::cerr << "ERROR: Failed to read power_sense - " << e.what()
                  << std::endl;
    } catch (...) {
        std::cerr << "ERROR: Failed to read power_sense - unknown error"
                  << std::endl;
    }
}
