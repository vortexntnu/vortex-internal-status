...
// MCP3422, MCP3423, MCP3426 & MCP3427 have two channels and treat 3 & 4 as
// repeats of 1 & 2 respectively MCP3424 & MCP3428 have all four channels
...
// 18-bit has a max sample rate of   3.75sps (MCP3421, MCP3422, MCP3423, MCP3424
// only)
...
class PowerSenseDriver {
   public:
    PowerSenseDriver(std::uint8_t i2c_bus,
                     std::uint8_t i2c_address,
                     std::uint8_t resolution);

    bool ping_powersense();
    bool start_conversion();
    std::uint16_t read_powersense(std::uint8_t channel);
